...
```{r vio1, fig.height=7, fig.width=9}
simu_violin(simu.results) +
  plot_annotation("All Method Comparison (phi=0.043)")
```
...
```{r vio2, fig.height=7, fig.width=9}
simu_violin(UDsimu.results) +
  plot_annotation("All Method Comparison (phi=0.043)")
```
...
```{r vio3, fig.height=7, fig.width=9}
simu_violin(rand_init.results) +
  plot_annotation("All Method Comparison (phi=0.043)")
```
...
# ----plotting params----
dir_name <- "./ASA_simulation_RDS/"
file_names <- c("BMPAMix_stop_crit.RDS") 
fname <- paste0(dir_name, file_names)
...
```{r vio4, fig.height=7, fig.width=9}
simu_violin(BMAPmix.more) +
  plot_annotation("All Method Comparison (phi=0.043)")
```

```{r s, results='asis', fig.height=5, fig.width=6*1.6}
n_methods <- length(method_names)
for (i in 1:n_methods) {
