...
Also, @ChoLuceWinterfeld1994 described PEST as:
...
#### Show sd and bias only 
```{r}
summ_table %>%
  select(!c(mean, median, true_value)) %>% 
  pivot_wider(names_from = variable,
              values_from = sd:mean_bias,
              names_glue = "{variable}_{.value}") %>% 
  relocate(contains("bias"), .after = n_bisect) %>% 
  rmarkdown::paged_table()
...
#### Full Trace plot for Bisection method
```{r}
converg.plot.bisec <- function(){
   plot_list <- list()
  # Some useful objects
  limit.df <- exp.result %>%
    bind_rows(.id = "Bisection_times") %>%
    summarise(
      across(L:x1neg,
             list(max = max,
                  min = min),
             .names = "{.col}_{.fn}"))
  for (i in 1:length(exp.result)) {
    df <- exp.result[[i]]
    df.name <- df %>% 
      pull(Bisection) %>%
      {\(x) max(x)-1}()
    ...
    for (pts in est) {
      ...
      ylimit <- c(limit.df[[glue("{pts}_min")]],
                limit.df[[glue("{pts}_max")]])
      ylimit <- round(ylimit/50)*50
      
      plt <- df %>%
        ggplot() +
        geom_line(
          aes(x = Bisection-1, y = !!sym(.y),
              # color = Nsim,
              group = Nsim),
          color = "grey50",
          linewidth = 1,
        ...
        geom_ribbon(data = CI.summ,
          aes(x = Bisection-1, ymin = !!sym(.y_CI[1]), ymax = !!sym(.y_CI[2])),
          fill = "cornflowerblue",
          alpha = .65
          ) +
        theme(legend.position = "none") +
        ...
        labs(title = pts,
             subtitle = glue("Bisection = {df.name}"),
             y = pts)
        
      plot_list[[length(plot_list) + 1]] <- plt
    }
  }
  combined_plot <- wrap_plots(plot_list, byrow = FALSE, ncol = 4) +
    plot_annotation(title = glue("Bisection Choice Trace"))
  combined_plot
}
converg.plot.bisec()
```
