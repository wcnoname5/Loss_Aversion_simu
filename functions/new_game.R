...
elicit_methods <- R6Class(
  ...
  public = list(
    ...
    init_bounds = function(bounds_manager, init_values, targets, fix_bnd_width) {
      # Default implementation - can be overridden
    }
  )
)
...
Bisection_strategy <- R6Class(
  ...
  public = list(
    ...
    init_bounds = function(bounds_manager, init_values, targets, fix_bnd_width) {
      bounds_manager$init_bounds(init_values, targets, fix_bnd_width)
    }
  )
)
# MOBS Strategy -------------------------------------------------------------
MOBS_strategy <- R6Class(
  ...
  public = list(
    ...
    init_bounds = function(bounds_manager, init_values, targets, fix_bnd_width) {
      ...
      bounds_manager$init_mobs_bounds(init_values, targets, fix_bnd_width)
    }
  )
)
...
bounds_manager <- R6Class(
  ...
  public = list(
    ...
    init_bounds = function(init_values, targets, fix_bnd_width = FALSE) {
...
task_logger <- R6Class(
  ...
  public = list(
    initialize = function(targets, init_values, random_init = FALSE, fix_bnd_width = FALSE) {
    ...
    prepare_chained_stim = function(cur_task_idx, random_init, fix_bnd_width = FALSE) {
      # function: convert estimated point to next task idx
      GetTargetIndex <- function(target_name) which(private$targets == target_name) + 1

      # Create correct bound list
      ...
      SetTaskBounds <- function(target_name, lower_bnd, upper_bnd) {
        ...
        private$bounds_manager$update_bounds(task_index, lower_bnd, upper_bnd)
      }


      # This function initializes the next chained target value in the task log.
      # Also sets the bounds for bisection-based methods.
      # outcomes are a list with left_pos, left_neg, right_stim
      InitializeNextTask <- function(
          chained_target, outcomes,
          ChainedTargetGain = FALSE, random_init = FALSE, fix_bnd_width = FALSE) {
      ...
      if (cur_task_idx == GetTargetIndex("L")) {
        ...
        InitializeNextTask("L2", outcomes, FALSE, random_init, fix_bnd_width)
        ...
        InitializeNextTask("G2", outcomes, TRUE, random_init, fix_bnd_width)
        ...
        cur_xi <- tail(self$show_task_log()[[cur_xi_name]], 1)
        loss_1 <- private$init_values[["l"]]
        L2 <- tail(self$show_task_log()[["L2"]], 1)
        ...
        InitializeNextTask(next_xi_name, outcomes, TRUE, random_init, fix_bnd_width)
        ...
        cur_xi <- tail(self$show_task_log()[[cur_xi_name]], 1)
        gain_1 <- private$init_values[["g"]]
        G2 <- tail(self$show_task_log()[["G2"]], 1)
        ...
        InitializeNextTask(next_xi, outcomes, FALSE, random_init, fix_bnd_width)
      }
    },
...
Game <- R6Class(
  ...
  public = list(
    ...
    update_task_log = function(choice, cur_task_idx, cur_trial, ...) {
      ...
      # If this is the first trial of a task, prepare chained stimulus
      ...
      # Handle Update late phase status if mixture
      if (private$is_mix) {
    ...
    prepare_chained_stim = function(cur_task_idx, random_init = FALSE, fix_bnd_width = FALSE) {
    ...
    get_current_step = function(cur_task_idx, cur_trial) {
      ...
      private$strategy$get_step(cur_task_idx, cur_trial, context)
    },
