...
file_names <- "Mixture_hyp_param.RDS"
fname <- file.path(dir_name, file_names)
pic_path <- "./figs/"

method_names <- c("Bisection_mixASA") |>
  outer(c(1000, 500, 300), paste, sep="_start")|>
  outer(c(50, 100, 150), paste, sep="_step")|>
...
process_method <- function(method_name, exp_param, mix_param, phi) {
  ...
  mix_param$start_crit <- 
    sub(".*_start(\\d*).*", "\\1", method_name) |>
    as.numeric()
  mix_param$start_step <- 
    sub(".*_step(\\d*).*", "\\1", method_name) |>
    as.numeric()
  if (tmp_method %in% c("ASA", "UD")) {
    mix_param$est_method <- tmp_method
  } 
  cat(glue::glue("start_crit: {mix_param$start_crit}, class: {class(mix_param$start_crit)}\n\n"))
  cat(glue::glue("start_step: {mix_param$start_step}, class: {class(mix_param$start_step)}\n\n"))
  stopifnot(is.numeric(mix_param$start_crit))
  stopifnot(is.numeric(mix_param$start_step))
