...
log_file <- file.path(log_dir, paste0("Study2-simulation", format(Sys.time(), "%Y%m%d_%H%M%S"), ".log"))
...
.phi <- 0.0367 # err = 1e-8, 0.027513126 is err = 1e-6 in L
...
process_method <- function(method_name, param, exp_param, mix_param, phi) {
  ...
  exp_param$fix_bnd_width <-
    grepl("Bisection|MOBS", method_name) & !grepl("SimpBisection", method_name)
  exp_param$random_init <- grepl("randInit", method_name)
  # Bisection_mixASA
  use_mixture <- grepl("mix", method_name)
  # Replace to the first captured group
