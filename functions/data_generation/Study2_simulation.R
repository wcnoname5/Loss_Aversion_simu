...
file_name <- paste0("Study2_simulation_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".rds")
...
log_file <- file.path(log_dir,
  paste0("Study2-simulation_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".log")
)
...
lambda_levels <- c(2.25) # Loss aversion parameter
...
method_names <-
    c(
        ...
        outer(c("SimpBisection", "PEST", "ASA"),
                    c("", "_randInit"),
                    function(x, y) paste0(x, y)),
        "Bisection_mixASA", "Bisection_mixASA_adaptiveBnd"
    ) |>
    outer(early_stop_times, paste, sep = "_early") |>
    as.vector()
...
process_method <- function(method_name, param, exp_param, mix_param, phi) {
    ...
    # Extract early stop parameter from method name
    ...
    # Adjust early stop for slider methods
    exp_param$early_stop <-
    ...
    for (p in param_list) {
        ...
        tmp_list <- lapply(seq_along(phi_vec), function(i) {
            ...
            tryCatch({
                ...
                write_process_log(glue("ER2ROR in processing phi = {phi} for {param_name}: {conditionMessage(e)}"))
