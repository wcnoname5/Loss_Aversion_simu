...
Player <- R6Class("Player",
  private = list(
    ...
    choose = function(lotteries) {  # Choose one of the lotteries depending on the player's utility function
      ...
      prospect.A <- private$compute_prospects(lotteries$A)
      prospect.B <- private$compute_prospects(lotteries$B)
      if (is.na(prospect.A)) {
        print(lotteries$A)
      ...
      cho_A_prob <- 1/( 1+exp(private$phi * prospect_dif) ) #softmax
      cA <- rbinom(1,1, cho_A_prob)
      if (cA){
        return("A")
      }else{
        return("B")
      }
      # if (prospect.A > prospect.B) {
      #   return("A")
      # } else if (prospect.A < prospect.B) {
      #   return("B")
      # } else {  # prospect.A == prospect.B
      #   choose_prob <- runif(1)
      #   if (choose_prob >= .5) {
      #     return("A")
      #   } else {
    ...
    compute_prospects = function(lottery) {
      ...
      if (lottery[1] == lottery[2]) {  # 100%
        if (lottery[1] >= 0) {
          mixed_prospect <- lottery[1] ^ private$alpha
          # mixed_prospect <- ((1+lottery[1]) ^ private$alpha - 1)/private$alpha
        } else {
          mixed_prospect <- -private$lambda * ((-lottery[2]) ^ private$beta)
          # mixed_prospect <- -private$lambda * ((1-lottery[2]) ^ private$beta - 1)/private$beta
        }
      } else {  # 50% vs. 50%
        gain_prospect <- lottery[1] ^ private$alpha
        # gain_prospect <- ((1+lottery[1]) ^ private$alpha - 1)/private$alpha
        loss_prospect <- - private$lambda * ((-lottery[2]) ^ private$beta)
        # loss_prospect <- -private$lambda * ((1-lottery[2]) ^ private$beta - 1)/private$beta
        mixed_prospect <- (gain_prospect + loss_prospect) * .5 # assume no weighting function
      }
...
experiment = function(alpha, beta, lambda, phi,
                      task_log=FALSE,
                      lotteries_box=FALSE) {  # Given player's attribute
  # Initialization
  player <- Player$new(alpha, beta, lambda, phi)
  game <- Game$new()
...
# curve(CRRA(x, .88), from =-5, to = 5 ,lwd= 2, col = "lightblue")
# curve(CARA(x, .5,.5), from =-5, to = 5 ,lwd= 2,lty=2, col = "pink", add = T)
